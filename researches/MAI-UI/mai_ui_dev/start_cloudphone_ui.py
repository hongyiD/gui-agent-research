...
def main():
    print("=" * 50)
    print("  MAI-UI Cloud Phone Web UI 启动器")
    print("=" * 50)
    print()
    ...
    # 检查 web_ui_cloudphone 目录
    web_ui_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "web_ui_cloudphone")
    if not os.path.exists(web_ui_dir):
        print(f"\n✗ web_ui_cloudphone 目录不存在: {web_ui_dir}")
        sys.exit(1)

    # 将 web_ui_cloudphone 和 src 目录添加到 Python 路径
    ...
    try:
        ...
        print(f"\n✗ 导入失败: {e}")
        print("请确保 web_ui_cloudphone/app.py 存在且正确")
        sys.exit(1)
    except Exception as e:
        print(f"\n✗ 启动失败: {e}")
        import traceback
